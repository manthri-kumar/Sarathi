...
router.post("/", async (req, res) => {
  try {
    ...
    if (inFlow && !C.looksLikeStepAnswer(s.step, raw)) {
      const answer = await C.askGemini(raw, city);
      // state preserved — re-prompt the current step
    ...
    if (!inFlow) {
      ...
      // general → natural Gemini answer (dual-mode, no flow active)
      const answer = await C.askGemini(raw, city);
    ...
    if (s.step === "destination") { s.trip.destination = C.clean(raw); return advance(s, res); }
    ...
    if (s.step === "train_class") {
      ...
      return finalizeTransport(s, res, { type: "train", option: "Train", klass, fare, source: "Distance Based", breakdown: null });
    }
    ...
    if (s.step === "bus_type") {
      ...
      return finalizeTransport(s, res, { type: "bus", option: type, klass: null, fare, source: "Distance Based", breakdown: null });
    }
    ...
    if (s.step === "flight_class") {
      ...
      return finalizeTransport(s, res, { type: "flight", option: klass, klass: null, fare, source: "Distance Based", breakdown: null });
    }
    ...
    if (s.step === "car_fuel") {
      ...
      if (f === "ev") {
        ...
        return finalizeTransport(s, res, { type: "car", option: "EV", klass: null, fare: b.total, source: "Distance Based", breakdown: b });
      }
    ...
    if (s.step === "car_mileage") {
      ...
      return finalizeTransport(s, res, { type: "car", option: s.trip.carFuelType.toUpperCase(), klass: null, fare: b.total, source: "Distance Based", breakdown: b });
    }
